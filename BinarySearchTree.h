...
int BinarySearchTree<T>::getHeight(TreeNode<T>* tNode)
{
   ...
   int l = getHeight(tNode->getLeft());
   int r = getHeight(tNode->getRight());
   if(l > r)
   {
	   return l + 1;
   }
...
bool BinarySearchTree<T>::isBalanced(TreeNode<T>* tNode)
{
   ...
   bool l = isBalanced(tNode->getLeft());
   bool r = isBalanced(tNode->getRight());
   ...
   if(fabs(left - right) > 1) //if absolute value of left - right is greater than one return true
   {
	   return true;
   }
